...
export default {
  ...
  mounted: function(){
    documento.querySelector('#skills').value = this.oldskills;
  },
  methods: {
    activar(e) {

      if (e.target.classList.contains("bg-teal-400")) {
        //el skill esta activo, quitarlo
        e.target.classList.remove('bg-teal-400');

          //eliminar al set de habilidades
          this.habilidades.delete(e.target.textContent);
      } else {
        //el skill no esta activo, ponerlo
        e.target.classList.add("bg-teal-400");
        //agregar al set de habilidades
        this.habilidades.add(e.target.textContent);
      }

      //agregar las habilidades al input
